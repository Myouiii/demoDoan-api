...
const getAddressApi = async (req, res, next) => {
  try{
    ...
    for(let i = 0; i < data.length; i++){
      const add = new AddressModel(data[i]);
      add.save();
    }
  }catch(err){
    console.log(err);
...
const getDistrict = async (req, res, next) => {
  try {
    ...
    const data = await AddressModel.findOne({ code }).select('districts -_id');
    if (data) {
...
const getWardStreetList = async (req, res, next) => {
  try {
    ...
    const data = await AddressModel.findOne({ code: id }).select('districts -_id');
    if (data) {
      const result = data.districts.find((item) => item.id == district);
      return res
        .status(200)
        .json({ wards: result.wards});
    }
  } catch (error) {
    return res.status(400).json({ message: 'failed' });
  }
